...
type CanonicalUserWithPassword = CanonicalUser & {
  password: string
}
...
function requireScriptPassword(envVar: string) {
  const value = process.env[envVar]?.trim()
  if (!value) {
    throw new Error(`Set ${envVar} before running prisma/backfill-users.ts`)
  }
...
async function upsertCanonicalUser(user: CanonicalUserWithPassword) {
